...
from fireducks.pandas import DataFrame as PandasDF
...
def test_results(q_num: int, result_df: PandasDF):
    with CodeTimer(name=f"Testing result of fireducks Query {q_num}", unit="s"):
...
def run_query(q_num: int, query: Callable):
    @linetimer(name=f"Overall execution of fireduks Query {q_num}", unit="s")
    def run():
